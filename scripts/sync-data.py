#!/usr/bin/env python3
"""
Sync data between cloud and local Supabase for Bible Study app.
...
Environment variables (for cloud):
    SUPABASE_URL — cloud Supabase URL (https://avctqylfozfsoavkmxgt.supabase.co)
...
TABLES = [
    "profiles", "books", "chapters", "sessions",
    "media", "notes", "quizzes", "discussions", "announcements",
]
...
def get_creds(env):
    ...
    url = os.environ.get("SUPABASE_URL", "https://avctqylfozfsoavkmxgt.supabase.co")
...
def fetch_table(url, key, table):
    req = urllib.request.Request(
        f"{url}/rest/v1/{table}?select=*&order=created_at",
...
def main():
    parser = argparse.ArgumentParser(description="Sync Bible Study data")
